const Selection = (function() {
    ...
    function _selection() {
        ...
        const translateConfig = {
            ...
            icon:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" enable-background="new 0 0 24 24" width="24" height="24" class="selection__icon">' +
        ...
        const cOneConfig = {
            url: "",
            icon:
                '<button class="btn btn-primary" id="colorOne" onclick="coneFunction()"></button>'
        };

        const cTwoConfig = {
            url: "",
            icon:
                '<button class="btn btn-primary" id="colorTwo" onclick="ctwoFunction()"></button>'
        };

        const cThreeConfig = {
            url: "",
            icon:
                '<button class="btn btn-primary" id="colorThree" onclick="cthreeFunction()"></button>'
        };

        const cFourConfig = {
            url: "",
            icon:
                '<button class="btn btn-primary" id="colorFour" onclick="cfourFunction()"></button>'
        };

        const cFiveConfig = {
            url: "",
            icon:
                '<button class="btn btn-primary" id="colorFive" onclick="cfiveFunction()"></button>'
        };
        ...
        function twitterButton() {
            ...
            const twbtn = new Button(twitterConfig.icon, function() {
                popupwindow(
                    twitterConfig.url + encodeURIComponent(text) + " " + url,
                    "Share",
                    550,
                    295
                );
        ...
        function searchButton() {
            const searchbtn = new Button(searchConfig.icon, function() {
                popupwindow(
                    searchConfig.url + encodeURIComponent(text),
                    "Search",
                    900,
                    540
                );
        ...
        function translateButton() {
            const tsbtn = new Button(translateConfig.icon, function() {
                popupwindow(
                    translateConfig.url + getBrowserLanguage() + "/" + text,
                    "Translate",
                    900,
                    540
                );
        ...
        function cOneButton() {
            const cOnebtn = new Button(cOneConfig.icon, function() {
                ...
                // para.setAttribute('class', 'test');
                // var node = document.createTextNode(text);
                // var app = para.appendChild(node);

                // console.log(para);

                // redColorArray.push(text);

                // //console.log(redColorArray.length);

                // for (let elements of redColorArray) {
        ...
        function appendIcons() {
            const myitems = [
                ...
                { feature: "translate", call: translateButton() },
                { feature: "copy", call: copyButton() },
                { feature: "speak", call: speakButton() },
        ...
        function setTooltipPosition() {
            ...
            left =
                position.left + (position.width - iconsize * _icons.length) / 2;
        }
        ...
        function config(options) {
            menu.twitter =
                options.twitter === undefined ? menu.twitter : options.twitter;
            menu.facebook =
                options.facebook === undefined
                    ? menu.facebook
                    : options.facebook;
            menu.search =
                options.search === undefined ? menu.search : options.search;
            menu.translate =
            ...
            menu.speak =
                options.speak === undefined ? menu.speak : options.speak;
            menu.disable =
                options.disable === undefined ? menu.disable : options.disable;
            menu.colorOne =
                options.colorOne === undefined
                    ? menu.colorOne
                    : options.colorOne;
            menu.colorTwo =
                options.colorTwo === undefined
                    ? menu.colorTwo
                    : options.colorTwo;
            menu.colorThree =
                options.colorThree === undefined
                    ? menu.colorThree
                    : options.colorThree;
            menu.colorFour =
                options.colorFour === undefined
                    ? menu.colorFour
                    : options.colorFour;
            menu.colorFive =
                options.colorFive === undefined
                    ? menu.colorFive
                    : options.colorFive;
